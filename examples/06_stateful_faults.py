...
for _ in range(8):
    mood.execute("select id from treats order by id").fetchall()
print(
    "mood遷移:",
...
returning.execute("select id, name from treats order by id").fetchall()
print(f"STASH直後のhouse件数: {len(returning.dolly.house())}")
returning.execute("pragma user_version").fetchall()
...
old_bone.execute("update treats set name = '新しいほね' where id = 1")
