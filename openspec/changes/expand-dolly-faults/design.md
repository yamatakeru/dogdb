...
- **付則（実装時確定）**: タグ統一とMVP決定値の1bit不変は両立しないため、STASH/SHUFFLE/IGNOREの既存導出のみv1互換経路（NUL区切りlabel等）を保持し、契約v2に互換規則として明記した。新規障害はすべて標準タグ導出を使う。
...
- **決定**: `wrap()` に (1) 全障害の重み辞書（既存 `faults` の拡張、未指定は0）、(2) `mood` 設定（有効化・epoch長・係数表）、(3) `auto_return` 設定、(4) `clock`、(5) スコーピング `only_tables` / `exclude_tables` を追加する。スコーピングは分類器のFROM句テーブル名抽出（保守的）に基づき、抽出できない文は `only_tables` 指定時は**対象外**（安全側）、`exclude_tables` 指定時は**対象**とする。両者の同時指定は意味が曖昧なため設定エラーとする（実装時確定）。すべてadditiveで、既定値はMVP挙動と完全一致。
