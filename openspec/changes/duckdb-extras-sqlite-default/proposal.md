...
- duckdbを`[project.optional-dependencies] duckdb = ["duckdb"]`へ移し、本体`dependencies`からduckdbを外す（sqlglot追加はW6-1の範囲であり、pyproject依存節の最終統合はwave集約時に親が行う）。
...
- sqlglotの追加、fingerprint正規化、POLICY_VERSION更新（W6-1の範囲）。POLICY_VERSIONはこのchangeでは触らない。
...
- **依存関係**: `pyproject.toml`の`dependencies`節はW6-1（sqlglot追加）と同一箇所を触りうる。コンフリクト解消はwave集約時に親が行う（統括issue #19）。
