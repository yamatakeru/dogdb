...
houseの状態（未返却の宝物集合）は、イベントログのSTASH/返却イベント列から完全に再構築できなければならない（MUST）。再構築結果と生きているhouse状態の一致は自動テストで検証されなければならない（SHALL）。

#### Scenario: ログからハウスを再建する
- **WHEN** STASHと部分返却が混在したセッションのイベントログを再生する
- **THEN** 再構築されたhouseの宝物集合（treasure_idと隠し状態）が実際のhouseと一致する
