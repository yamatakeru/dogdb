...
3. **SQLite表面の `with` 意味論**: ブロック終了時の無条件 close → commit/rollback（close しない）。close に依存していたコードは、明示的に `close()` しない限り接続とその保持リソースを持ち続ける。
...
- [エンジン抽出のリファクタで介入コアの挙動が変わる回帰リスク] → エンジン抽出単独の時点で既存テストスイートを無改変のまま通し（tasks 1.3の回帰ゲート）、表面分岐後も決定性・イベント列一致のコアアサーションを維持する（表面契約に関わるテストの書き換えは許容）。
