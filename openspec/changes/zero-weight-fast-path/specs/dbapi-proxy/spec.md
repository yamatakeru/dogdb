...
- **WHEN** 全障害の重みを0にしてラップした接続でSELECTを複数回実行する
...
全障害の実効重みが0の設定における小さなSELECTの反復実行で、ラップ接続のオーバーヘッドは生接続の5倍以内でなければならない（SHALL）。その測定手順は再現可能なベンチマークスクリプトとしてリポジトリに常置しなければならない（SHALL）。
