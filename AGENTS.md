...
- PR作成後はConversation上でレビューが実行される。レビューはpushごとにhead commitのcheckとして走るため、指摘の収集はそのcheckの完了を待ってから行う（スレッドへの解決返信の新着は完了シグナルではない）。Nitpicksを含むすべての指摘について妥当性を確認し（必要に応じてサブエージェントを使う）、必要な修正を行い、各指摘への対応をコメントで返す。これを指摘がなくなるまで繰り返す。
