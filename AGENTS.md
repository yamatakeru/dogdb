...
- 複数changeにまたがる可変の状態（wave依存関係・並列可能性・起票予定・進捗）は統括issueで管理する。PR連動とクローズにより状態の鮮度が自動で保たれるためで、キャンペーン作業の着手時はまず統括issueを読む。
...
- `git worktree add ../<repo>-<略称> -b <changeブランチ> <集約ブランチ>` で分離し、実装は各ワークツリー内で実装ワーカーへ委譲する。プロンプト先頭を `/opsx:apply <change名>` にしてスキルを発火させる。
